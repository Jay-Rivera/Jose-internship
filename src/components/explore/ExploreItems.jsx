...
const ExploreItems = () => {
  ...
  async function getExploreItemsOnFilter(event) {
    setIsLoading(true);
    const { data } = await axios.get(
    ...
    <>
      <div>
        <select
          id="filter-items"
          defaultValue=""
          onChange={getExploreItemsOnFilter}
        >
      ...
      {!isLoading ? (
        <>
          {exploreItems.slice(0, itemsToLoad).map((item, index) => (
            <div
              key={index}
      ...
      <div className="col-md-12 text-center">
        <Link
          onClick={() => setItemsToLoad(itemsToLoad + 4)}
          to=""
          id="loadmore"
